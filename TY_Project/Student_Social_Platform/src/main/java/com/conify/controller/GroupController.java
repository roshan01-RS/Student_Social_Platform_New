...
public class GroupController {
        ...
        try {
            ...
            if (joinDate != null) {
                return ResponseEntity.ok(
                        allMessages.stream()
                                .filter(m -> !m.getTimestamp().isBefore(joinDate))
    ...
    // =========================================================
    // 🔥 MESSAGE SEND (REPLY SUPPORT ADDED — NOTHING BROKEN)
    // =========================================================
    ...
    public ResponseEntity<?> sendMessage(
        ...
        try {
            ...
            msg.setType(
                    msg.getMediaUrl() != null
                            ? GroupMessage.MessageType.IMAGE
                            : GroupMessage.MessageType.TEXT
            );

            // 🔥 REPLY FIELDS (SAFE OPTIONAL ADDITION)
            msg.setReplyToMessageId(payload.get("replyToMessageId"));
            msg.setReplyToSenderName(payload.get("replyToSenderName"));
            msg.setReplyToContent(payload.get("replyToContent"));

            messageRepository.save(msg);

            group.setLastMessage(
                    msg.getType() == GroupMessage.MessageType.IMAGE
                            ? "📷 Photo"
                            : msg.getContent()
            );
            group.setLastUpdated(Instant.now());
    ...
    public ResponseEntity<?> addMembers(
        ...
        try {
            ...
            for (Long id : newMemberIds) {
                if (!group.getMemberIds().contains(id)) {
                    ...
                    notificationService.createNotification(
                            currentUserId,
                            id,
                            Notification.NotificationType.GROUP_ADD,
                            "added you to " + group.getName(),
                            groupId,
                            null
                    );
                }
            ...
            if (!addedNames.isEmpty()) {
                createSystemMessage(groupId,
                        getUsername(currentUserId) + " added " + String.join(", ", addedNames));
                groupRepository.save(group);
            }
        ...
        try {
            ...
            if (group.getJoinRequests().contains(targetUserId)) {
                ...
                if (accept) {
                    ...
                    createSystemMessage(groupId,
                            getUsername(approverId) + " added " + getUsername(targetUserId));

                    notificationService.createNotification(
                            approverId,
                            targetUserId,
                            Notification.NotificationType.GROUP_ADD,
                            "approved your join request for " + group.getName(),
                            groupId,
                            null
                    );
                }
